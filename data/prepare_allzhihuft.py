...
if not os.path.exists("all_zhihu_finetune"):
    os.makedirs("all_zhihu_finetune")

np.array(train_data, dtype=np.uint16).tofile(os.path.join("all_zhihu_finetune", "train.bin"))
np.array(val_data, dtype=np.uint16).tofile(os.path.join("all_zhihu_finetune", "val.bin"))
